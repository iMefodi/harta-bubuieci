/* Portal WebGIS Bubuieci — limba RO/EN, harti interactive (capitole + taburi), lightbox foto,
   meniu mobil, galerie, animatii */
(function () {
  'use strict';

  // ---------------- traduceri (RO = textul din HTML, EN = mai jos) ----------------
  var EN = {
    'nav.story': 'Story', 'nav.maps': 'Interactive maps', 'nav.charts': 'Charts', 'nav.gallery': 'Gallery',
    'nav.links': 'Useful links', 'nav.contact': 'Contact',
    ...
    'card1.t': 'The Bubuieci story', 'card1.d': 'Five chapters on what the village looks like today: land, houses, services and networks.',
    'card2.t': 'Interactive maps', 'card2.d': 'Click any building, parcel or shop and see its details.',
    'card3.t': 'Numbers and charts', 'card3.d': 'Land, buildings and the pace of construction, explained simply.',
    'card4.t': 'Photo gallery', 'card4.d': 'Real pictures of the village, from the town hall to the fields.',
    'more': 'Learn more ›',
    ...
    'stats.src': 'Sources: population — NBS (attributes of the village boundary layer); area and lengths — GIS computation in QGIS; buildings — cadastre, sector 0142; points of interest and roads — OpenStreetMap, 2026.',
    'story.h': 'The Bubuieci story. <span>In five maps.</span>',
    'story.p': 'Five chapters about the village next to Chișinău. Every map is interactive: press it, then zoom in and click on features.',
    'map.activate': 'Explore the interactive map', 'map.full': 'Full screen ↗',
    'c1.n': 'Chapter 1 · Location', 'c1.t': 'Why Bubuieci?',
    ...
    'c2.n': 'Chapter 2 · Land use', 'c2.t': 'A village of fields',
    ...
    'c3.n': 'Chapter 3 · Built-up area', 'c3.t': 'House after house',
    ...
    'c4.n': 'Chapter 4 · Services and transport', 'c4.t': 'Everyday life',
    ...
    'c5.n': 'Chapter 5 · Infrastructure', 'c5.t': 'The networks that keep the village alive',
    ...
    'maps.p': 'Five thematic maps, each showing only its own topic, plus the complete project map and the QGIS Cloud version. Zoom in, switch layers on and off in the legend, click on features.',
    'tab.localizare': 'Location', 'tab.terenuri': 'Land use', 'tab.cladiri': 'Buildings', 'tab.servicii': 'Services',
    'tab.infrastructura': 'Infrastructure',
    'md.localizare': 'Where Bubuieci lies relative to Chișinău. Click the village boundary for population and area.',
    'md.terenuri': 'How every hectare is used: fields, orchards, forests, built-up areas. Click an area for its type and size.',
    'md.cladiri': 'All buildings and parcels from the cadastre. Click a building for its use, cadastral code and house number.',
    'md.servicii': 'Shops, pharmacies, bus stops and playgrounds. Search by name or type in the left panel.',
    'md.infrastructura': 'Streets, the Bîc river and the power network. Click a street for its name and length.',
    'md.leaflet': 'The complete map with every project layer: layer list on the right, ruler and search on the left.',
    'md.cloud': 'The map published on QGIS Cloud · public WMS/WFS service: wms.qgiscloud.com/Eduard_Iftodii/bubuieci_webgis/',
    ...
    'g1.src': 'Source: OpenStreetMap (landuse), clipped to the village boundary, computed in QGIS.',
    ...
    'g4.src': 'Source: Copernicus CORINE Land Cover, clipped to the village boundary (official CLC colours).',
  ...
  // ---------------- harti: adresele fiecarei harti ----------------
  var CLOUD = 'https://qgiscloud.com/Eduard_Iftodii/bubuieci_webgis/';
  function mapUrl(key, embed, nofull) {
    if (key === 'leaflet') return 'harta/index.html';
    if (key === 'cloud') return CLOUD;
    return 'tematice/?t=' + key + (embed ? '&embed=1' : '') + (nofull ? '&nofull=1' : '') + '&lang=' + lang;
  }
  // trimite limba catre hartile tematice deja deschise (acelasi domeniu)
  function tellMaps() {
    document.querySelectorAll('iframe').forEach(function (fr) {
      if ((fr.getAttribute('src') || '').indexOf('tematice/') === 0 && fr.contentWindow) {
        fr.contentWindow.postMessage({ type: 'lang', lang: lang }, location.origin);
      }
    });
  }
  ...
  function setLang(l) {
    ...
    // descrierea tabului curent isi schimba cheia dinamic
    var desc = document.getElementById('map-desc');
    if (desc) desc.innerHTML = t(desc.getAttribute('data-i18n'));
    altNodes.forEach(function (el) { el.setAttribute('alt', t(el.getAttribute('data-i18n-alt'))); });
    document.querySelectorAll('.lang button').forEach(function (b) {
      b.classList.toggle('on', b.getAttribute('data-lang') === lang);
    });
    document.querySelectorAll('.map-full[href^="tematice/"]').forEach(function (a) {
      a.href = a.getAttribute('href').replace(/&lang=\w+/, '').replace(/$/, '&lang=' + lang);
    });
    var mf = document.getElementById('map-full');
    var cur = document.querySelector('.maptabs button.on');
    if (mf && cur) mf.href = mapUrl(cur.getAttribute('data-map'), false);
  ...
  // ---------------- harti din capitole: previzualizare -> harta interactiva la click ----------------
  document.querySelectorAll('.ch-map').forEach(function (box) {
    var btn = box.querySelector('.activate');
    btn.addEventListener('click', function () {
      var fr = document.createElement('iframe');
      fr.src = mapUrl(box.getAttribute('data-theme'), true);
      fr.title = box.querySelector('img').alt;
  ...
  // ---------------- sectiunea de harti: taburi care schimba harta afisata ----------------
  var frame = document.getElementById('mapframe');
  var desc = document.getElementById('map-desc');
  var full = document.getElementById('map-full');
  function showMap(key) {
    document.querySelectorAll('.maptabs button').forEach(function (b) { b.classList.toggle('on', b.getAttribute('data-map') === key); });
    frame.src = mapUrl(key, true, true);
    frame.setAttribute('data-map', key);
    desc.setAttribute('data-i18n', 'md.' + key);
    desc.innerHTML = t('md.' + key);
    full.href = mapUrl(key, false);
  }
  document.querySelectorAll('.maptabs button').forEach(function (b) {
    b.addEventListener('click', function () { showMap(b.getAttribute('data-map')); });
  });
  frame.src = mapUrl('localizare', true, true);   // loading="lazy": se incarca abia cand ajungi la sectiune
  ...
  document.querySelectorAll('.ch-photo, .ph').forEach(function (fig) {
  ...
  document.querySelectorAll('[data-scroll]').forEach(function (b) {
    b.addEventListener('click', function () {
      var sc = document.getElementById(b.getAttribute('data-target'));
